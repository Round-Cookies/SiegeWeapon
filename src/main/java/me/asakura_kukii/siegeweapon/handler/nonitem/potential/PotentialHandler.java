...
import me.asakura_kukii.siegeweapon.utility.format.UnicodeHandler;
...
public class PotentialHandler {
    ...
    public static String formPotentialBar (String s) {
        ...
        for (char c : charList) {
            if (c == '_') {
                sB.append(UnicodeHandler.map(SiegeWeapon.potentialNormal));
            }
            if (c == '=') {
                sB.append(UnicodeHandler.map(SiegeWeapon.potentialGenerate));
            }
            if (c == '+') {
                sB.append(UnicodeHandler.map(SiegeWeapon.potentialUpdate));
            }
            if (c == '*') {
                sB.append(UnicodeHandler.map(SiegeWeapon.potentialVisionNormal));
            }
            if (c == '#') {
                sB.append(UnicodeHandler.map(SiegeWeapon.potentialVisionGenerate));
            }
            if (c == '%') {
                sB.append(UnicodeHandler.map(SiegeWeapon.potentialVisionUpdate));
            }
        }
