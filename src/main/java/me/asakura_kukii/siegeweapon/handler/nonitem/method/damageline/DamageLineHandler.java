package me.asakura_kukii.siegeweapon.handler.nonitem.method.damageline;
...
import me.asakura_kukii.siegeweapon.handler.nonitem.method.common.MethodNodeData;
import me.asakura_kukii.siegeweapon.handler.nonitem.sound.SoundData;
import me.asakura_kukii.siegeweapon.handler.nonitem.sound.SoundHandler;
import me.asakura_kukii.siegeweapon.loader.common.FileType;
import me.asakura_kukii.siegeweapon.utility.boundingbox.BoundingBox;
import me.asakura_kukii.siegeweapon.utility.boundingbox.BoundingBoxHandler;
import me.asakura_kukii.siegeweapon.handler.nonitem.particle.ParticleData;
import me.asakura_kukii.siegeweapon.handler.nonitem.block.BlockHandler;
import me.asakura_kukii.siegeweapon.handler.nonitem.particle.ParticleHandler;
...
public class DamageLineHandler extends MethodHandler {
    ...
    public static void damageLine(List<MethodNodeData> mNDL, Location start, Vector direction, Entity shooter, DamageLineData dLD, HashMap<String, Object> extraData) {
        ...
        for (double dist = 0; dist <= dLD.length * rM; dist = dist + SiegeWeapon.blockCheckSpacing) {
            ...
            if (SiegeWeapon.breakableBlockMap.containsKey(m) && !blocksHit.containsKey(current)) {
                blocksHit.put(current, dist);
                objectsHit.put(dist, current);

            } else {
                RayTraceResult rTR = Objects.requireNonNull(current.clone().getWorld()).rayTraceBlocks(current.clone(), direction.clone().normalize(), SiegeWeapon.blockCheckSpacing, FluidCollisionMode.NEVER, true);
                if (rTR != null) {
                    finalDistance = dist;
        ...
        if (!objectsHit.isEmpty()) {
            ...
            for(Object o: keySet) {
                ...
                if (value instanceof Location) {
                    ...
                    if (SiegeWeapon.penetrableBlockMap.containsKey(m)) {
                        double blockDamage = damage * SiegeWeapon.penetrableBlockMap.get(m);
                        double remainingDamage = 0;
                        //check remaining damage;
                        double health = BlockHandler.blockHealthMapper.get(l.getBlock().getLocation()) * SiegeWeapon.breakableBlockMap.get(m);
                        ...
                        BlockHandler.blockHealthMapper.put(l.getBlock().getLocation(), health / SiegeWeapon.breakableBlockMap.get(m));
                    ...
                    if (bB != null) {
                        ...
                        if (bB.headShotCheck(start.clone(), direction.clone())) {
                            entityDamage *= SiegeWeapon.headShotMultiplier;
                            headShotSound = true;
        ...
        double trailTransition = SiegeWeapon.trailTransitionStart;
        for (double dist = SiegeWeapon.trailBias; dist <= finalDistance; dist = dist + SiegeWeapon.trailSpacing + trailTransition) {
            trailTransition *= SiegeWeapon.trailTransitionFactor;
            Location current = start.clone().add(direction.clone().normalize().multiply(dist));
