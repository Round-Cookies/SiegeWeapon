package me.asakura_kukii.siegeweapon.utility.command;

import me.asakura_kukii.siegeweapon.SiegeWeapon;
import me.asakura_kukii.siegeweapon.handler.item.ItemData;
...
import me.asakura_kukii.siegeweapon.handler.nonitem.potential.PotentialHandler;
import me.asakura_kukii.siegeweapon.handler.nonitem.potential.PotentialType;
...
public class CommandHandler {
    ...
    public static boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args[0].equalsIgnoreCase("item")) {
            sender.sendMessage("");
            sender.sendMessage(SiegeWeapon.pluginPrefix + "Issued command:");
            sender.sendMessage(ChatColor.WHITE + ">> " + commandPainter(args));
            if (sender instanceof Player) {
                if (sender.hasPermission("siegeweapon.item")) {
                    //ana item XXX
                    if (args.length>=2) {
                        if (args[1].equalsIgnoreCase("list")) {
                            if (args.length>=3) {
                                if (args[2].equalsIgnoreCase("gun")) {
                                    ...
                                    sender.sendMessage(SiegeWeapon.pluginPrefix + ChatColor.RED + "COMMAND ERROR - Invalid sub-command");
                                    return true;
                                }
                            } else {
                                sender.sendMessage(SiegeWeapon.pluginPrefix + ChatColor.RED + "COMMAND ERROR - Missing sub-command");
                                return true;
                            }
                    ...
                    sender.sendMessage(SiegeWeapon.pluginPrefix + "PERMISSION ERROR - Missing permission");
                    return true;
                }
            } else {
                //ana item XXX
                    ...
                    if (args.length>=2) {
                        if (GunData.gunDataMapper.containsKey(args[1])) {
                            if (args.length>=3) {
                                    ...
                                    if (silencer <= 2 && silencer >= 0 && grip <= 2 && grip >= 0 && scope <= 2 && scope >= 0) {
                                        if (args.length==3) {
                                            if(PlayerHandler.playerDataMapper.containsKey(((Player) sender).getUniqueId())) {
                                                ...
                                                sender.sendMessage(SiegeWeapon.pluginPrefix + ChatColor.RED + "SERVER ERROR - Invalid player, contact OP");
                                                return true;
                                            }
                                        } else {
                                            if (sender.hasPermission("siegeweapon.admin")) {
                                                if (Bukkit.getPlayer(args[3])!=null && Bukkit.getOnlinePlayers().contains(Bukkit.getPlayer(args[3]))) {
                                                    ...
                                                    if(PlayerHandler.playerDataMapper.containsKey(p.getUniqueId())) {
                                                        ...
                                                        sender.sendMessage(SiegeWeapon.pluginPrefix + ChatColor.RED + "VARIABLE ERROR - Invalid player");
                                                        return true;
                                                    }
                                                } else {
                                                    sender.sendMessage(SiegeWeapon.pluginPrefix + ChatColor.RED + "VARIABLE ERROR - Invalid player");
                                        ...
                                        sender.sendMessage(SiegeWeapon.pluginPrefix + ChatColor.RED + "VARIABLE ERROR - Invalid modification");
                                        return true;
                                    }
                                ...
                                sender.sendMessage(SiegeWeapon.pluginPrefix + ChatColor.RED + "VARIABLE ERROR - Missing modification");
                                return true;
                            }
                        } else {
                            sender.sendMessage(SiegeWeapon.pluginPrefix + ChatColor.RED + "VARIABLE ERROR - Invalid item name");
                            return true;
                        }
                    } else {
                        sender.sendMessage(SiegeWeapon.pluginPrefix + ChatColor.RED + "VARIABLE ERROR - Missing item name");
                        return true;
                    }
                } else {
                    sender.sendMessage(SiegeWeapon.pluginPrefix + "PERMISSION ERROR - Missing permission");
                            ...
                            try {
                                ...
                                if (silencer <= 2 && silencer >= 0 && grip <= 2 && grip >= 0 && scope <= 2 && scope >= 0) {
                                    if (args.length>=4) {
                                        ...
                                        sender.sendMessage(SiegeWeapon.pluginPrefix + "PERMISSION ERROR - Missing player");
                                        return true;
                                    }
                                } else {
                                    sender.sendMessage(SiegeWeapon.pluginPrefix + ChatColor.RED + "VARIABLE ERROR - Invalid modification");
        ...
        if (args[0].equalsIgnoreCase("reload")) {
            ...
            sender.sendMessage(ChatColor.WHITE + ">> " + commandPainter(args));
            if (sender instanceof Player) {
                if (sender.hasPermission("siegeweapon.reload")) {
                    Main.getInstance().reloadConfig();
                    Main.getInstance().reloadValues();
                    sender.sendMessage(ChatColor.GRAY + ">> " + ChatColor.WHITE + "PLUGIN RELOADED");
                    return true;
                } else {
                    sender.sendMessage(SiegeWeapon.pluginPrefix + ChatColor.RED + "PERMISSION ERROR - Missing permission");
                    return true;
                }
            } else {
                Main.getInstance().reloadConfig();
        ...
        if (args[0].equalsIgnoreCase("util")) {
            ...
            sender.sendMessage(ChatColor.WHITE + ">> " + commandPainter(args));
            if (sender instanceof Player) {
                if (sender.hasPermission("siegeweapon.util")) {
                    if (args.length >= 2) {
                        if (args[1].equalsIgnoreCase("spawn")) {
                            if (args.length >= 3) {
                                ...
                                sender.sendMessage(SiegeWeapon.pluginPrefix + ChatColor.RED + "VARIABLE ERROR - Invalid entity");
                                return true;
                            }
                            sender.sendMessage(SiegeWeapon.pluginPrefix + ChatColor.RED + "VARIABLE ERROR - Missing entity");
                            return true;
                        }
                            ...
                            if (args.length >= 3) {
                                try {
                                    ...
                                    try {
                                        ...
                                        sender.sendMessage(SiegeWeapon.pluginPrefix + ChatColor.RED + "ITEM ERROR");
                                        return true;
                                    }
                                } catch (Exception ignored) {
                                    sender.sendMessage(SiegeWeapon.pluginPrefix + ChatColor.RED + "VARIABLE ERROR - Invalid index");
                                    return true;
                                }
                            }
                            sender.sendMessage(SiegeWeapon.pluginPrefix + ChatColor.RED + "VARIABLE ERROR - Missing index");
                            return true;
                        }
                        sender.sendMessage(SiegeWeapon.pluginPrefix + ChatColor.RED + "COMMAND ERROR - Invalid sub-command");
                        return true;
                ...
                sender.sendMessage(SiegeWeapon.pluginPrefix + ChatColor.RED + "COMMAND ERROR - Only player can run this command");
                return true;
            }
            return true;
        }
                            ...
                            try {
                                ...
                                if (args.length>=7) {
                                    try {
                                        ...
                                        if (args.length>=8) {
                                            try {
                                                ...
                                                if (level >= 1 && level <= 30) {
                                                    ...
                                                    sender.sendMessage(SiegeWeapon.pluginPrefix + ChatColor.RED + "VARIABLE ERROR - Invalid level");
                                                }
                                            } catch (Exception ignored) {
                                                sender.sendMessage(SiegeWeapon.pluginPrefix + ChatColor.RED + "VARIABLE ERROR - Invalid level");
                                            }
                                        ...
                                        sender.sendMessage(SiegeWeapon.pluginPrefix + ChatColor.RED + "VARIABLE ERROR - Invalid slot");
                                    }
                                } else {
                                ...
                                sender.sendMessage(SiegeWeapon.pluginPrefix + ChatColor.RED + "VARIABLE ERROR - Invalid amount");
                            }
                        } else {
        ...
        if (args.length == 1) {
            ...
            if (args[0].equalsIgnoreCase("modify")) {
                ...
                if (GunData.gunDataMapper.containsKey(args[1])) {
                    ...
                    if (args.length == 4 && sender.hasPermission("siegeweapon.admin")) {
