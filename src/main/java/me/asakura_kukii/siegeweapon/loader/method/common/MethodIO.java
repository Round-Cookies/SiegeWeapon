package me.asakura_kukii.siegeweapon.loader.method.common;
...
public abstract class MethodIO {
    public static MethodNodeData loadTree(FileConfiguration fC, String fileName) {
        if (fC.contains("methodTree")) {
            ...
            FileIO.fileMessageMapper.put(fileName, FileIO.fileMessageMapper.get(fileName) + SiegeWeapon.consolePluginPrefix + "methodTree is empty\n");
        }
        return null;
    }
    ...
    public static MethodNodeData loadNode(ConfigurationSection cS, MethodNodeData mND, String fileName) {
        ...
        SiegeWeapon.server.getConsoleSender().sendMessage(mT + dataList.toString() + mND.trigger);


        if (cS.contains("child") && cS.getConfigurationSection("child") != null) {
            ConfigurationSection childCS = cS.getConfigurationSection("child");
            assert childCS != null;
            SiegeWeapon.server.getConsoleSender().sendMessage(childCS.getKeys(false).toString());
            for (String s: childCS.getKeys(false)) {
                ...
                if (!mND.mT.triggerList.contains(s)) {
                    FileIO.fileStatusMapper.put(fileName, false);
                    FileIO.fileMessageMapper.put(fileName, FileIO.fileMessageMapper.get(fileName) + SiegeWeapon.consolePluginPrefix + currentPath + s + " is not valid trigger\n");
                } else {
